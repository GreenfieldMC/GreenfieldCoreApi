...
begin
    insert into `ScriptManager.ScriptHistory` (IsInit, AppliesTo, Major, Minor)
...
begin
    ...
    select case
               ...
               when p_IsInit = 1 then (select count(*) from `ScriptManager.ScriptHistory` sh where sh.AppliesTo = p_AppliesTo) = 0
               -- We are not an init script, so check if there are scripts applied for the same AppliesTo
               when (select count(*) from `ScriptManager.ScriptHistory` sh where sh.AppliesTo = p_AppliesTo) > 0 then
                   -- There are scripts applied for the same AppliesTo, so check the versioning
                   case
                       when (select max(sh.Major) from `ScriptManager.ScriptHistory` sh where sh.AppliesTo = p_AppliesTo) < p_Major then 1
                       when (select max(sh.Major) from `ScriptManager.ScriptHistory` sh where sh.AppliesTo = p_AppliesTo) = p_Major
                           and (select max(sh.Minor) from `ScriptManager.ScriptHistory` sh where sh.AppliesTo = p_AppliesTo and sh.Major = p_Major) < p_Minor then 1
                       else 0
                       end
               else 0
               end as ShouldApply;
